...
- **vision**: A vision-capable subagent (`@vision` or via the task tool) that looks at images, screenshots, mockups, diagrams, and charts and reports their contents. It exists ONLY for models without native vision: if you can see the image yourself (natively), read it directly and never delegate. Delegate to `vision` only when the active model cannot see attachments — e.g. a Read/attachment call fails with "model does not support image input" — then relay its report back to the user rather than guessing.
